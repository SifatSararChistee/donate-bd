document.getElementById("donate-btn3").addEventListener("click", function () {
  const amount = inputFieldValueById("amount-input3");
  const balance = getInnerTextById("balance");
  const donatedMoney = getInnerTextById("donate-counter3");
  if (isNaN(amount)) {
  ...
  document.getElementById("donate-counter3").innerText = NewDonatedMoney;

  const history = document.createElement("div");
  history.innerHTML = `
            <div class="border-2 border-[#1111111A] shadow-xl rounded-2xl p-5 mb-6">
            <h3 class="text-lg font-bold">
              ${amount} Taka is Donated for Aid for Injured in the Quota Movement, Bangladesh
            </h3>
